...
enum class SymbolState : uint8_t {
  ...
  Emitted,           /// Emitted to memory, but waiting on transitive deps.
  Ready,             /// Symbol and transitive dependencies emitted.
  Initialized = 0x3f /// Initializers run, safe for clients to access.
};
...
private:
  ...
  void notifyInitialized(const SymbolNameVector &Initialized);
...
public:
  ...
  /// This method will be called for each symbol that becomes ready. If there
  /// are any pending initializers then the ready symbols can be held until the
  /// initializers are run before they are moved to the initialized state.
  virtual void notifyReady(JITDylib &JD, SymbolNameVector Symbols) = 0;
...
protected:
  /// Update the symbol state for the given symbols to initialized.
  static void notifySymbolsInitialized(JITDylib &JD,
                                       const SymbolNameVector &Symbols) {
    JD.notifyInitialized(Symbols);
  }
};
...
public:
  ...
  Expected<SymbolMap>
  lookup(const JITDylibSearchOrder &SearchOrder, const SymbolLookupSet &Symbols,
         LookupKind K = LookupKind::Static,
         SymbolState RequiredState = SymbolState::Initialized,
         RegisterDependenciesFunction RegisterDependencies =
